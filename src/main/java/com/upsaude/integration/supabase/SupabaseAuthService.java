...
public class SupabaseAuthService {
        ...
        try {
            ...
            if (e.getStatusCode() == HttpStatus.UNAUTHORIZED ||
                ...
                try {
                    ...
                    if (errorBody != null && errorBody.contains("message")) {
                        ...
                        String errorMessage = error.getMessage() != null ? error.getMessage() : "Credenciais inválidas";
            ...
            if (cause != null && (cause instanceof com.fasterxml.jackson.databind.exc.MismatchedInputException ||
                ...
                throw new RuntimeException(
                        "Erro ao processar resposta do serviço de autenticação: " + cause.getMessage(), e);
            }
     ...
     * Use este método quando o access token expirar para evitar que o usuário
     * precise fazer login novamente.
     * 
     * @param refreshToken Token de atualização obtido no login
     * @return Nova resposta de autenticação com novo access_token e refresh_token
     */
    public SupabaseAuthResponse refreshToken(String refreshToken) {
        ...
        log.debug("Renovando token no Supabase. URL: {}", url);
        ...
        try {
            log.debug("Enviando requisição de refresh token para: {}", url);
            ResponseEntity<SupabaseAuthResponse> response = restTemplate.exchange(
                    url,
                    HttpMethod.POST,
                    request,
            ...
            log.warn("Falha ao renovar token - Status: {}, Body: {}",
                    response.getStatusCode(), response.getBody());
            throw new UnauthorizedException("Falha ao renovar token");

        } catch (HttpClientErrorException e) {
            log.error("Erro HTTP ao renovar token - Status: {}, Body: {}",
                    e.getStatusCode(), e.getResponseBodyAsString());

            if (e.getStatusCode() == HttpStatus.UNAUTHORIZED ||
                    e.getStatusCode() == HttpStatus.BAD_REQUEST) {
                // Refresh token inválido ou expirado - usuário precisa fazer login novamente
                throw new UnauthorizedException("Sessão expirada. Por favor, faça login novamente.");
            }

            throw new UnauthorizedException("Erro ao renovar token: " + e.getMessage());
        } catch (Exception e) {
            log.error("Erro inesperado ao renovar token", e);
            throw new RuntimeException("Erro ao renovar token: " + e.getMessage(), e);
        }
    }
