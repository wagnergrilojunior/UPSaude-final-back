package com.upsaude.service.support.servicosestabelecimento;

import com.upsaude.api.request.ServicosEstabelecimentoRequest;
import com.upsaude.exception.BadRequestException;
import org.springframework.stereotype.Service;
import org.springframework.util.StringUtils;

@Service
public class ServicosEstabelecimentoValidationService {

    public void validarObrigatorios(ServicosEstabelecimentoRequest request) {
        if (request == null) {
            throw new BadRequestException("Dados do serviço do estabelecimento são obrigatórios");
        }
        if (request.getEstabelecimento() == null) {
            throw new BadRequestException("Estabelecimento é obrigatório");
        }
        if (!StringUtils.hasText(request.getNome())) {
            throw new BadRequestException("Nome do serviço é obrigatório");
        }
    }
}

