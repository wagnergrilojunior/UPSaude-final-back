...
public class ResponsavelLegalValidationService {
    ...
    public void validarObrigatorios(ResponsavelLegalRequest request) {
        if (request == null) {
            throw new BadRequestException(ValidationMessages.get("validation.responsavel.legal.obrigatorio"));
        }
        if (request.getPaciente() == null) {
            throw new BadRequestException(ValidationMessages.get("validation.responsavel.legal.id.obrigatorio"));
        }
        if (request.getEstabelecimento() == null) {
            throw new BadRequestException(ValidationMessages.get("validation.estabelecimento.id.obrigatorio"));
        }
        if (!StringUtils.hasText(request.getNome())) {
            throw new BadRequestException(ValidationMessages.get("validation.responsavel.legal.nome.obrigatorio"));
        }
    }
    ...
    public String normalizarCpf(String cpf) {
        ...
        if (StringUtils.hasText(cpfLimpo) && cpfLimpo.length() != 11) {
            throw new BadRequestException(ValidationMessages.get("validation.cpf.digitos"));
        }
        return cpfLimpo;
    }
    ...
    public String normalizarTelefone(String telefone) {
        ...
        if (StringUtils.hasText(telefoneLimpo) && telefoneLimpo.length() != 10 && telefoneLimpo.length() != 11) {
            throw new BadRequestException(ValidationMessages.get("validation.telefone.digitos"));
        }
