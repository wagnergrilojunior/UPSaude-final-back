...
class TituloReceberRegressionTest extends BaseRegressionTest {
    ...
    private UUID criarPlanoContas() throws Exception {
        String nome = "Plano para Titulo " + System.currentTimeMillis();
    ...
    private UUID criarPagador() throws Exception {
        String jsonPayload = """
                {
                  "tipo": "PAGADOR",
                  "nome": "Pagador Teste"
    ...
    void criarTituloParaTestes() throws Exception {
        ...
        pagadorId = criarPagador();

        String numero = String.format("TR%06d", System.currentTimeMillis() % 1000000L);
        LocalDate hoje = LocalDate.now();
        String jsonPayload = String.format("""
                {
                  "pagador": "%s",
                  "contaContabilReceita": "%s",
                  "numero": "%s",
                ...
                """, pagadorId, contaContabilId, numero, hoje, hoje.plusDays(30));

        MvcResult result = mockMvc.perform(post("/api/v1/financeiro/titulos-receber")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(jsonPayload))
                .andExpect(status().isCreated())
                .andReturn();
    ...
    @Test
    void cadastroMinimoDeTituloReceberNaoPodeQuebrar() throws Exception {
        UUID planoId = criarPlanoContas();
        UUID contaId = criarContaContabil(planoId);
        UUID pagador = criarPagador();
        String numero = String.format("TR%06d", System.currentTimeMillis() % 1000000L);
        LocalDate hoje = LocalDate.now();
        ...
        String jsonPayload = String.format("""
                ...
                """, pagador, contaId, numero, hoje, hoje.plusDays(30));

        mockMvc.perform(post("/api/v1/financeiro/titulos-receber")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(jsonPayload))
                .andExpect(status().isCreated());
    }
    ...
    @Test
    void listagemDeTitulosReceberNaoPodeQuebrar() throws Exception {
        mockMvc.perform(get("/api/v1/financeiro/titulos-receber")
                        .param("page", "0")
                        .param("size", "10"))
    ...
    @Test
    void obterTituloReceberPorIdNaoPodeQuebrar() throws Exception {
        mockMvc.perform(get("/api/v1/financeiro/titulos-receber/" + tituloId))
                .andExpect(status().isOk());
    }
}
